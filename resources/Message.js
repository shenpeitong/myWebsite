...
try {
	var d = parent.document.domain;
} catch (e) {
	try {
		document.domain = "12366.cn";
	} catch (e) {}
	try {
		document.domain = "tax.cn";
	} catch (e) {}
	try {
		document.domain = "sat.tax";
	} catch (e) {}
}
(function() {
	...
	while (t = initFn.shift()) Message[t] = eval('0,function(){_initFn.' + t + '?_initFn.' + t + '.push(arguments):(_initFn.' + t + '=[arguments])}');
	/*以下为公用函数及变量*/
	var isIE = !+'\v1'; //IE浏览器
	...
	var addEvent = (function() {
		return new Function('env', 'fn', 'obj', 'obj=obj||document;' + (window.attachEvent ? "obj.attachEvent('on'+env,fn)": 'obj.addEventListener(env,fn,false)') + ';Message.eventList.push([env,fn,obj])')
	})(); //事件绑定
	...
	var mkBtn = function(txt, sign, autoClose, id) {
		...
		id = id || 'Message_btn_' + seed++;
		autoClose = autoClose == undefined ? 'undefined': !!autoClose;				//特殊处理		if(txt=='确定（仍要享受减免）' || txt=='取消（不需享受减免）'){			return {				id: id,				html: "<input type='button' id='" + id + "'" +((curCfg && curCfg.btnAbleDelay)?" disabled=true ":"")+" onclick='Message.doHandler(\"" + sign + "\"," + autoClose + ")' style='cursor:pointer' class='btnStyle1 handler' value='" + txt + "' />"			};		}
		return {
			id: id,
			html: "<input type='button' id='" + id + "'" +((curCfg && curCfg.btnAbleDelay)?" disabled=true ":"")+" onclick='Message.doHandler(\"" + sign + "\"," + autoClose + ")' style='cursor:pointer' class='btnStyle handler' value='" + txt + "' />"
		};
	};
	/*生成按钮组合的html*/
	...
	var dftCfg = {
		...
		btnAbleDelay:0,  //按钮出现延长时间
		closeTxt: '关闭',		//关闭按钮的提示文本
		okTxt: ' 确 定 ',		//确定按钮的提示文本
		cancelTxt: ' 取 消 ',	//取消按钮的提示文本
		msgCls: 'msg-content',	//消息框内容的class名称，用于自定义样式，默认为msg-content,仅在iframe:false时有效
		minBtn: false,			//是否显示最小化按钮，默认不显示
		minTxt: '最小化',		//最小化按钮的提示文本
		maxBtn: false,			//是否显示最大化按钮，默认不显示
		maxTxt: '最大化',		//最大化按钮的提示文本
		allowSelect:true,		//是否允许选择消息框内容，默认不允许
		allowRightMenu:true	//是否允许在消息框使用右键，默认不允许
	},curCfg = {};
	...
	(function() {
		...
		/*窗体*/
		"<div id='msg-window' style='position:absolute;z-index:10001;display:none'>", IE6 ? "<iframe src='javascript:false' style='width:100%;height:100%;position:absolute;top:0;left:0;z-index:-1'></iframe>": '', "<div class='msg-tl' id='msg-tl'><div class='msg-tr'><div class='msg-tc' style='cursor:move;'><div class='msg-header-text'></div><div class='msg-header-tools'>", "<div class='Message_min' title='最小化'><strong>0</strong></div>", "<div class='Message_max' title='最大化'><strong>1</strong></div>", "<div class='Message_close' title='关闭'><strong>r</strong></div>", "</div></div></div></div>", "<div class='msg-ml' id='msg-ml'><div class='msg-mr'><div class='msg-mc'><div class='msg-body' style='position:relative'></div></div></div></div>", "<div class='msg-ml' id='msg-btnl'><div class='msg-mr'><div class='msg-btn'></div></div></div>", "<div class='msg-bl' id='msg-bl'><div class='msg-br'><div class='msg-bc'></div></div></div>", "</div>",
		/*阴影*/
		isIE ? "<div id='msg-shadow' style='position:absolute;z-index:10000;background:#808080;filter:alpha(opacity=80) progid:DXImageTransform.Microsoft.Blur(pixelradius=2);display:none'></div>": ''].join('');
		document.body.appendChild(div);
		/*窗口上的对象*/
		/*mask、window*/
		var maskLevel = $('maskLevel');
		var ym_win = $('msg-window');
		var ym_shadow = $('msg-shadow');
		var ym_wins;
		/*header*/
		var ym_headbox = $('msg-tl');
		var ym_head = ym_headbox.firstChild.firstChild;
		var ym_hText = ym_head.firstChild;
		var ym_hTool = ym_hText.nextSibling;
		/*content*/
		var ym_body = $('msg-ml').firstChild.firstChild.firstChild;
		/*button*/
		var ym_btn = $('msg-btnl');
		var ym_btnContent = ym_btn.firstChild.firstChild;
		/*bottom*/
		var ym_bottom = $('msg-bl');
		...
		var doMax = function() {
			...
			ym_ico[1].className = 'Message_normal';
		...
		var doMin = function() {
			...
			ym_ico[0].className = 'Message_normal';
		...
		var doNormal = function(init) { //init=true,弹出时调用该函数
			...
			ym_ico[0].className = 'Message_min';
			ym_ico[1].className = 'Message_max';
		...
		addEvent('mousedown',function(e) {
			...
			if (curCfg.iframe) {
				...
				isCompat && IE6 && Message.apply(cfg, {
		...
		var winVisible = function(visible) {
			...
			if (visible === false) { //关闭
				...
				var closeFn = function() {
					...
					if (cacheWin.length) Message.show.apply(null, cacheWin[0].concat(true))
				};
				/*渐变方式关闭*/
			...
			ym_head.className = 'msg-tc' + (curCfg.titleBar ? '': ' msg-ttc'); //无标题栏
			...
			/*iframe如果不加上opacity=100，则msg-win和用于遮罩iframe的div也透明时，iframe也就透明了*/
			...
			ym_body.className = "msg-body " + curCfg.icoCls; //图标类型
		...
		Message.apply(Message, {
			...
			show: function(args, fargs, show) { //如果有窗口未关闭则将本次传入的信息放到队列里
				...
				if (!isObj(a[0])) {
					...
					o = a[0];
                    if(isArray(o.message)){
                        var newMessage = "";
                        var height = 0;
                        for(var i=0;i<o.message.length;i++){
                           newMessage+=(i+1)+":"+o.message[i]+"<br/>";
                           height+=18;
                        }
				...
				Message.apply(curCfg, Message.apply({}, o, fargs), Message.setDefaultCfg()); //先还原默认配置
				/*修正curCfg中的无效值(null/undefined)改为默认值*/
				for (var i in curCfg) curCfg[i] = curCfg[i] != null ? curCfg[i] : Message.cfg[i];
				init();
			},
			...
			setDefaultCfg: function(cfg) {
				return Message.cfg = Message.apply({}, cfg, Message.apply({}, Message.cfg, dftCfg));
			},
			getButtons: function() {
		...
		Message.setDefaultCfg(); //初始化默认配置
		/*执行用户初始化时的调用*/
		var t;
		for (var i in _initFn) while (t = _initFn[i].shift()) Message[i].apply(null, t);
		if(curCfg.btnAbleDelay) {
			setTimeout(function(){
				showBtn(btnCache);
			},curCfg.btnAbleDelay*1000);
		}
		/*取消事件绑定*/
		addEvent('unload', function() {
			while (Message.eventList.length) detachEvent.apply(null, Message.eventList.shift());
		}, window);
		
		function showBtn(tButtons) {
			if(!tButtons)return;
			try {
				for(var i=0;i<tButtons.length;i++) {
					var tButton = tButtons[i];
					document.getElementById(tButton.id).disabled = false;
				}
			}catch(e) {}
		}
	})();
...
Message.apply(Message, {
	alert: function() {		if(Message.cfg==null||Message.cfg == "null" || Message.cfg == undefined || Message.cfg == "undefined"){//Message.cfg如果为undefined，先不处理，解决脚本报错					}else{			Message.cfg.height=185;		}
		if(arguments[0].length /29-2>0)
			Message.cfg.height=15*Math.ceil(arguments[0].length /29)+185;
		if(arguments[0].length /29-6>0){
			Message.cfg.height=385;
		}
		Message.show(arguments, {
			icoCls: 'Message_alert',
			btn: ['OK']
		});
	},
